...
export class AppController implements Controller {
  ...
  selectCar(carId: number, carName: string, carColor: string) {
    const inputCarName: HTMLInputElement = document.querySelector("#input__car-update-name");
    const inputCarColor: HTMLInputElement = document.querySelector("#input__car-update-color");
  ...
  async startRace(raceCars: number[]) {
    ...
    Promise.allSettled(racePromises)
      ...
      .then((winner) => {
        const finishLogo = document.getElementById(`img__race-finish-${winner.id}`);
